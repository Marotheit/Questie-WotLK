...
function Questie:ClearConfig(arg)
	...
	StaticPopupDialogs["CLEAR_CONFIG"] = {
		...
		OnAccept = function()
			...
			-- Questie will read the quests based on what is in your log. A refresh of sorts without
...
QuestieFastSlash = {
	...
	["query"] = function()
	-- Default: None - Popup confirmation of Yes or No - Popup has a 60 second timeout.
		StaticPopupDialogs["QUERY_SERVER"] = {
			text = "|cFFFFFF00You are about to begin querying the server for completed quests.|n|nThis will delete all previous quest data and repopulate it within this character's SavedVariables.|n|nThis function is rate limited to once every two minutes.|n|nAre you sure you want to continue?|r",
			button1 = TEXT(YES),
			button2 = TEXT(NO),
			OnAccept = function()
				-- Clears all quests
				...
				-- Clears all tracked quests
				QuestieTrackedQuests = {}
				-- Query for completed quests
				local completedQuests = {}
				DEFAULT_CHAT_FRAME:AddMessage("Querying the server for completed quests. This may take a moment and freeze your game while performing the query.", 1, 1, 1);
				QueryQuestsCompleted()
				DEFAULT_CHAT_FRAME:AddMessage("Query complete. Populating quests...", 1, 1, 1);
	...
	["help"] = function()
		...
		DEFAULT_CHAT_FRAME:AddMessage("|c0000c0ff/questie query |r-- Queries the server for completed quests. Only possible every 15 minutes.", 0.75, 0.75, 0.75);
